...
function gameLoad() // initiate
{
	...
	document.getElementById("button").style.opacity = 0.5; // Start button
...
function buttonClick() // GO click
{
	...
	window.setInterval(carCheck, 100); // check car every X mSec
...
function move(dir)
{
	...
	function Drive()
	{
		...
		{
			if ((dir == 'left') && (element.style.left > '12%'))
				element.style.left = (parseInt(element.style.left) - 1 ) + '%'; // move left
				
			if ((dir == 'right') && (element.style.left < '75%'))
				element.style.left = (parseInt(element.style.left) + 1 ) + '%'; // move right
		}
	}
}
